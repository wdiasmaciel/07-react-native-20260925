...
const { promisify } = _nodeUtil.default;
const moveCursor = promisify(_nodeReadline.default.moveCursor);
const clearScreenDown = promisify(_nodeReadline.default.clearScreenDown);
...
function getTTYStream(stream) {
  if (
    stream instanceof _nodeTty.default.WriteStream &&
    stream.isTTY &&
    stream.columns >= 1
  ) {
    return stream;
...
class Terminal {
  ...
  log(format, ...args) {
    const line = _nodeUtil.default.format(format, ...args);
    if (this.#ttyStream) {
