...
async function main(
  ...
  try {
    ...
    let context;
    if (_nodeFs.default.lstatSync(sourceMapFileName).isDirectory()) {
      ...
      const content = _nodeFs.default.readFileSync(sourceMapFileName, "utf8");
    ...
    if (argv.length === 0) {
      ...
      stdout.write(
        JSON.stringify(
          context.symbolicateHeapSnapshot(
            _nodeFs.default.readFileSync(argv[0], "utf8"),
          ),
        ),
      );
    } else if (argv[0] === "--attribution") {
      let lineBuffer = "";
      const streamToLines = new _nodeStream.Transform({
      ...
      const symbolicateLines = new _nodeStream.Transform({
