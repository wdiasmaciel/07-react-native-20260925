...
class TreeFS {
  ...
  *matchFiles(opts) {
    ...
    const contextRootAbsolutePath =
      rootRealPath === ""
        ? this.#rootDir
        : _nodePath.default.join(this.#rootDir, rootRealPath);
    const prefix = filterComparePosix ? "./" : "." + _nodePath.default.sep;
    const contextRootAbsolutePathForComparison =
      filterComparePosix && _nodePath.default.sep !== "/"
        ? contextRootAbsolutePath.replaceAll(_nodePath.default.sep, "/")
    ...
    for (const relativePathForComparison of this.#pathIterator(
      ...
      if (
        ...
        const relativePath =
          filterComparePosix === true && _nodePath.default.sep !== "/"
            ? relativePathForComparison.replaceAll("/", _nodePath.default.sep)
            : relativePathForComparison;
        yield _nodePath.default.join(contextRootAbsolutePath, relativePath);
      }
    }
  }
  addOrModify(mixedPath, metadata, changeListener) {
    ...
    const parentDirNode = this.#lookupByNormalPath(
      _nodePath.default.dirname(normalPath),
      {
        changeListener,
    ...
    const canonicalPath = this.#normalizePath(
      parentDirNode.canonicalPath +
        _nodePath.default.sep +
        _nodePath.default.basename(normalPath),
    );
  ...
  bulkAddOrModify(addedOrModifiedFiles, changeListener) {
    ...
    for (const [normalPath, metadata] of addedOrModifiedFiles) {
      const lastSepIdx = normalPath.lastIndexOf(_nodePath.default.sep);
  ...
  #removeNormalPath(normalPath, changeListener) {
    ...
    if (isDirectory(node) && node.size > 0) {
      for (const basename of node.keys()) {
        this.#removeNormalPath(
          canonicalPath + _nodePath.default.sep + basename,
          changeListener,
        );
      }
      return;
    ...
    if (parentNode != null) {
      ...
      parentNode.delete(_nodePath.default.basename(canonicalPath));
      if (parentNode.size === 0 && parentNode !== this.#rootNode) {
        this.#removeNormalPath(
          _nodePath.default.dirname(canonicalPath),
          changeListener,
        );
      }
    }
  ...
  #lookupByNormalPath(
    ...
    while (targetNormalPath.length > fromIdx) {
      const nextSepIdx = targetNormalPath.indexOf(
        _nodePath.default.sep,
        fromIdx,
      );
        ...
        if (
          ...
          for (
            ...
            collapsedPath =
              collapsedPath === ""
                ? ".."
                : collapsedPath + _nodePath.default.sep + "..";
          }
  ...
  hierarchicalLookup(mixedStartPath, subpath, opts) {
    ...
    while (isDirectory(nextNode)) {
      ...
      if (maybeAbsolutePathMatch != null) {
        ...
        return {
          absolutePath: maybeAbsolutePathMatch,
          containerRelativePath: relativeParts.join(_nodePath.default.sep),
        };
      }
      ...
      candidateNormalPath =
        candidateNormalPath === ""
          ? ".."
          : candidateNormalPath + _nodePath.default.sep + "..";
  ...
  *#metadataIterator(rootNode, opts, prefix = "") {
    for (const [name, node] of rootNode) {
      ...
      const prefixedName =
        prefix === "" ? name : prefix + _nodePath.default.sep + name;
      if (isDirectory(node)) {
  ...
  #normalizePath(relativeOrAbsolutePath) {
    return _nodePath.default.isAbsolute(relativeOrAbsolutePath)
  ...
  *#pathIterator(
    ...
    const pathSep = opts.alwaysYieldPosix ? "/" : _nodePath.default.sep;
      ...
      if (!isDirectory(node)) {
        if (isRegularFile(node)) {
          ...
          const nodePathWithSystemSeparators =
            pathSep === _nodePath.default.sep
              ? nodePath
              : nodePath.replaceAll(pathSep, _nodePath.default.sep);
          const normalPathOfSymlink = _nodePath.default.join(
  ...
  #resolveSymlinkTargetToNormalPath(symlinkNode, canonicalPathOfSymlink) {
    ...
    let absoluteSymlinkTarget = _nodePath.default.resolve(
      this.#rootDir,
      canonicalPathOfSymlink,
      "..",
      literalSymlinkTarget,
    );
    if (absoluteSymlinkTarget.endsWith(_nodePath.default.sep)) {
    ...
    const result = {
      ...
      startOfBasenameIdx:
        normalSymlinkTarget.lastIndexOf(_nodePath.default.sep) + 1,
    };
