...
const debug = (0, _debug.default)("Metro:DeltaCalculator");
const changeEventIds = new WeakMap();
class DeltaCalculator extends _nodeEvents.default {
  ...
  #shouldReset(canonicalPath, metadata) {
    ...
    if (
      ...
      (canonicalPath === "package.json" ||
        canonicalPath.endsWith(_nodePath.default.sep + "package.json"))
    ) {
      return true;
    }
    return false;
  ...
  _handleMultipleFileChanges = (changeEvent) => {
    ...
    for (const [canonicalPath, metadata] of changes.addedFiles) {
      ...
      const absolutePath = _nodePath.default.join(rootDir, canonicalPath);
    ...
    for (const [canonicalPath, metadata] of changes.removedFiles) {
      ...
      const absolutePath = _nodePath.default.resolve(rootDir, canonicalPath);
      if (this._addedFiles.has(absolutePath)) {
    ...
    if (changeId == null) {
      changeId = _nodeCrypto.default.randomUUID();
