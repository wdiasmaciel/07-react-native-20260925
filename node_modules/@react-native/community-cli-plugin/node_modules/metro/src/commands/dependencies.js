...
async function dependencies(args, config) {
  ...
  if (!_nodeFs.default.existsSync(rootModuleAbsolutePath)) {
    return Promise.reject(
      new Error(`File ${rootModuleAbsolutePath} does not exist`),
    );
  }
  config.cacheStores = [];
  const relativePath = _nodePath.default.relative(
  ...
  const outStream =
    args.output != null
      ? _nodeFs.default.createWriteStream(args.output)
      : process.stdout;
  ...
  return args.output != null
    ? (0, _nodeUtil.promisify)(outStream.end).bind(outStream)()
    : Promise.resolve();
}
var _default = () => ({
