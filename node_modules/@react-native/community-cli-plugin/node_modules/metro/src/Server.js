...
const debug = (0, _debug.default)("Metro:Server");
...
class Server {
  constructor(config, options) {
    ...
    this._sourceRequestRoutingMap = [
      [
        "/[metro-project]/",
        _nodePath.default.resolve(this._config.projectRoot),
      ],
      ...this._config.watchFolders.map((watchFolder, index) => [
        `/[metro-watchFolders]/${index}/`,
        _nodePath.default.resolve(watchFolder),
      ]),
    ];
    this._isEnded = false;
    this._fetchTimings = [];
    ...
    const bundleOptions = {
      ...
      runBeforeMainModule:
        this._config.serializer.getModulesRunBeforeMainModule(
          _nodePath.default.relative(this._config.projectRoot, entryPoint),
        ),
  ...
  async _processSingleAssetRequest(req, res) {
    ...
    if (!assetPath && urlObj.searchParams.get("unstable_path")) {
      ...
      if (secondaryQuery) {
        Object.entries(_nodeQuerystring.default.parse(secondaryQuery)).forEach(
          ([key, value]) => {
    ...
    try {
      ...
      res.setHeader(
        "Content-Type",
        _mimeTypes.default.contentType(_nodePath.default.basename(assetPath)),
      );
  ...
  async _processSourceRequest(relativeFilePathname, rootDir, res) {
    ...
    const depGraph = await this._bundler.getBundler().getDependencyGraph();
    const filePath = _nodePath.default.join(rootDir, relativeFilePathname);
    try {
    ...
    const mimeType = _mimeTypes.default.contentType(
      _nodePath.default.basename(relativeFilePathname),
    );
  ...
  _createRequestProcessor({
    ...
    return async function requestProcessor(
      ...
      const requestStartTimestamp =
        _nodePerf_hooks.performance.timeOrigin +
        _nodePerf_hooks.performance.now();
    ...
    build: async ({
      ...
      const bundle = await serializer(
        ...
        revision.graph,
        {
          ...
          runBeforeMainModule:
            this._config.serializer.getModulesRunBeforeMainModule(
              _nodePath.default.relative(this._config.projectRoot, entryFile),
            ),
          runModule: serializerOptions.runModule,
          sourceMapUrl: serializerOptions.sourceMapUrl,
          sourceUrl: serializerOptions.sourceUrl,
  ...
  async _symbolicate(req, res) {
    ...
    const getCodeFrame = (urls, symbolicatedStack) => {
      ...
      for (let i = 0; i < symbolicatedStack.length; i++) {
        ...
        const fileAbsolute = _nodePath.default.resolve(
          this._config.projectRoot,
          file ?? "",
        );
  ...
  _resolveWatchFolderPrefix(filePath) {
    ...
    if (watchFolderMatch != null) {
      ...
      if (watchFolder != null) {
        return {
          rootDir: _nodePath.default.resolve(watchFolder),
          filePath:
            "." +
            _nodePath.default.sep +
            watchFolderMatch[2].split("/").join(_nodePath.default.sep),
        };
      }
    }
    ...
    if (projectMatch != null) {
      return {
        rootDir: _nodePath.default.resolve(this._config.projectRoot),
        filePath:
          "." +
          _nodePath.default.sep +
          projectMatch[1].split("/").join(_nodePath.default.sep),
      };
    }
    return null;
  }
  ...
  _getEntryPointAbsolutePath(entryFile) {
    ...
    if (resolved != null) {
      return _nodePath.default.resolve(resolved.rootDir, resolved.filePath);
    }
    return _nodePath.default.resolve(this._getServerRootDir(), entryFile);
  }
  async ready() {
