...
function saveAsAssets(bundle, options, log) {
  ...
  const modulesDir = _nodePath.default.join(
    _nodePath.default.dirname(bundleOutput),
    MODULES_DIR,
  );
...
function createDir(dirName) {
  return _nodeFs.promises.mkdir(dirName, {
    recursive: true,
  });
}
function writeModuleFile(module, modulesDir, encoding) {
  ...
  return (0, _writeFile.default)(
    _nodePath.default.join(modulesDir, id + ".js"),
    code,
    encoding,
  );
}
...
function writeMagicFlagFile(outputDir) {
  ...
  return (0, _writeFile.default)(
    _nodePath.default.join(outputDir, MAGIC_RAM_BUNDLE_FILENAME),
    buffer,
  );
}
